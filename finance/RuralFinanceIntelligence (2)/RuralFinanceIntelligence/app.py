...
def load_recommendation_engine():
    ...
    try:
        ...
        if model_files:
            ...
            # Return engine without pre-trained models
...
def profile_input_page():
    ...
    st.markdown("Please provide your financial and demographic information for personalized recommendations.")
    ...
    with col2:
        education = st.selectbox("Education Level", 
                                ["Illiterate", "Primary", "Secondary", "Graduate", "Post Graduate"])
        state = st.selectbox("State", 
                            ["Bihar", "Punjab", "Uttar Pradesh", "Maharashtra", 
                             "Karnataka", "Tamil Nadu", "Gujarat", "Rajasthan", "Other"])
        occupation = st.selectbox("Primary Occupation", 
                                 ["Marginal Farmer", "Irrigated Farmer", "Daily Wage Laborer", 
                                  "Small Business Owner", "Government Employee", "Private Employee", "Retired"])
    ...
    with col1:
        monthly_income_primary = st.number_input("Monthly Primary Income (₹)", 
                                                min_value=1000, max_value=500000, value=25000)
        monthly_income_secondary = st.number_input("Monthly Secondary Income (₹)", 
                                                  min_value=0, max_value=200000, value=5000)
        land_owned = st.number_input("Land Owned (Acres)", min_value=0.0, max_value=100.0, value=2.0)
        
    with col2:
        total_debt = st.number_input("Total Debt Amount (₹)", min_value=0, max_value=5000000, value=50000)
        gold_grams = st.number_input("Gold Owned (Grams)", min_value=0, max_value=1000, value=50)
        bank_distance = st.number_input("Distance to Bank (KM)", min_value=0.1, max_value=50.0, value=5.0)
    ...
    with col2:
        financial_goal = st.selectbox("Primary Financial Goal", 
                                    ["Emergency Fund", "Child Education", "House Purchase", 
                                     "Retirement", "Daughter's Marriage", "Asset Purchase"])
        monthly_investment_budget = st.number_input("Monthly Investment Budget (₹)", 
                                                   min_value=500, max_value=100000, value=5000)
    
    # Digital and Banking
    st.subheader("💳 Banking & Digital Access")
    col1, col2 = st.columns(2)
    
    with col1:
        has_bank_account = st.checkbox("Has Bank Account", value=True)
        uses_digital_payments = st.checkbox("Uses UPI/Digital Payments", value=True)
        
    with col2:
        has_smartphone = st.checkbox("Has Smartphone", value=True)
        internet_access = st.checkbox("Has Internet Access", value=True)
    
    # Save profile
    if st.button("💾 Save Profile & Get Recommendations", type="primary"):
        user_profile = {
            'Age': age,
            'Gender': gender,
            ...
            'Monthly_Income_Primary': monthly_income_primary,
            'Monthly_Income_Secondary': monthly_income_secondary,
            'Land_Owned_Acres': land_owned,
            'Total_Debt_Amount': total_debt,
            'Gold_Grams': gold_grams,
            'Bank_Distance_KM': bank_distance,
            'Risk_Tolerance': risk_tolerance,
            'Investment_Horizon': investment_horizon,
            'Financial_Goal': financial_goal,
            'Monthly_Investment_Capacity': monthly_investment_budget,
            'UPI_Usage': uses_digital_payments,
            'Smartphone_Access': has_smartphone,
            'Internet_Access': internet_access,
            'Bank_Account_Type': 'Regular Savings' if has_bank_account else 'None'
        }
        
        st.session_state.user_profile = user_profile
        st.success("✅ Profile saved successfully! Go to Recommendations page to see your personalized advice.")

def market_data_page():
    """Market data display page"""
    st.header("📈 Live Market Data")
    st.markdown("Current market conditions and investment opportunities")
    ...
    if market_data:
        ...
        if 'stocks' in market_data and market_data['stocks']:
            ...
            stock_df = pd.DataFrame(market_data['stocks'])
            if not stock_df.empty:
                st.dataframe(stock_df, use_container_width=True)
        
        # FD Rates section
        if 'fd_rates' in market_data and market_data['fd_rates']:
            st.subheader("🏦 Fixed Deposit Rates")
            for bank, rates in market_data['fd_rates'].items():
                with st.expander(f"{bank}"):
                    rates_df = pd.DataFrame(rates)
                    st.dataframe(rates_df, use_container_width=True)
        
        # Government Schemes
        if 'government_schemes' in market_data and market_data['government_schemes']:
            st.subheader("🏛️ Government Schemes")
            for scheme, details in market_data['government_schemes'].items():
                with st.expander(f"{scheme} - {details.get('full_name', 'N/A')}"):
                    col1, col2 = st.columns(2)
                    with col1:
                        if 'interest_rate' in details and details['interest_rate']:
                            st.metric("Interest Rate", f"{details['interest_rate']}%")
                        if 'min_investment' in details:
                            st.metric("Min Investment", f"₹{details['min_investment']:,}")
                    with col2:
                        if 'lock_in_period_years' in details and details['lock_in_period_years']:
                            st.metric("Lock-in Period", f"{details['lock_in_period_years']} years")
                        if 'max_investment' in details and details['max_investment']:
                            st.metric("Max Investment", f"₹{details['max_investment']:,}")
...
def recommendations_page():
    ...
    if not st.session_state.user_profile:
        st.warning("⚠️ Please complete your profile first in the Profile Input section.")
        return
    ...
    if engine is None:
        st.error("Unable to load recommendation engine. Please ensure models are trained.")
        return
    
    # Generate recommendations
    with st.spinner("Generating personalized recommendations..."):
        try:
            recommendations = engine.generate_comprehensive_recommendations(st.session_state.user_profile)
            ...
            if recommendations:
                ...
                if 'specific_products' in recommendations:
                        ...
                        with st.expander(f"{category.replace('_', ' ').title()}"):
                            for item in items:
                                ...
                                if 'expected_return' in item:
                                    st.write(f"Expected Return: {item['expected_return']:.1f}%")
                                if 'recommended_amount' in item:
                                    st.write(f"Recommended Amount: ₹{item['recommended_amount']:,}")
                ...
                if 'risk_analysis' in recommendations:
                    ...
                    with col1:
                        st.metric("Risk Score", f"{risk_analysis.get('overall_risk_score', 0)}/10")
                    with col2:
                        st.metric("Volatility", f"{risk_analysis.get('portfolio_volatility', 0):.1f}%")
                    with col3:
                        st.metric("Expected Return", f"{risk_analysis.get('expected_return', 0):.1f}%")
...
def model_training_page():
    ...
    dataset_files = [f for f in os.listdir('RuralFinanceIntelligence (2)/RuralFinanceIntelligence') 
                     if f.endswith('.csv') and 'dataset' in f.lower()]
    
    if dataset_files:
        latest_dataset = max(dataset_files, key=lambda x: os.path.getctime(f'RuralFinanceIntelligence (2)/RuralFinanceIntelligence/{x}'))
        try:
            df = pd.read_csv(f'RuralFinanceIntelligence (2)/RuralFinanceIntelligence/{latest_dataset}')
            col1, col2, col3 = st.columns(3)
            with col1:
            ...
            if st.button("🚀 Start Training", type="primary"):
                with st.spinner("Training ML models... This may take several minutes."):
                    try:
                        ...
                        trainer.train_complete_system(latest_dataset)
        ...
        if st.button("🔄 Generate Sample Dataset", type="secondary"):
            with st.spinner("Generating sample dataset..."):
                try:
                    ...
                    dataset = generator.generate_ultimate_dataset(num_rows=10000)
                    
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    filename = f"sample_dataset_{timestamp}.csv"
                    dataset.to_csv(filename, index=False)
                    
                    os.chdir('../..')
                    
                    st.success(f"✅ Sample dataset generated: {filename}")
...
def main():
    ...
    if st.session_state.user_profile:
        ...
        st.sidebar.write(f"**Name:** {st.session_state.user_profile.get('Occupation', 'N/A')}")
        st.sidebar.write(f"**Income:** ₹{st.session_state.user_profile.get('Monthly_Income_Primary', 0):,}")
    else:
    ...
    # Footer
    st.sidebar.markdown("---")
    st.sidebar.markdown("**AI Financial Advisor v1.0**")
    st.sidebar.markdown("Built for Rural India 🇮🇳")
